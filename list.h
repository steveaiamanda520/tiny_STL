...
/*这两个模板结构体list_node_base和list_node可能是用于实现链表的基本节点的结构体。
它们之间的区别可能在于继承关系和功能的扩展。

list_node_base<T>：这个模板结构体可能是一个基本的链表节点结构体，用于存储数据和指向下一个节点的指针。
通常，它可能包含以下成员：

数据成员：用于存储节点的数据。
指针成员：指向下一个节点的指针。

list_node<T>：这个模板结构体可能是对基本节点结构的扩展，可能包含更多的成员或者提供额外的功能。
可能的扩展包括：

在基本节点结构的基础上添加额外的数据成员。
提供额外的方法或操作，如插入、删除等。
可能会继承自list_node_base<T>，并在此基础上扩展功能。*/
...
struct node_traits
{
  typedef list_node_base<T>* base_ptr;   //基本链表节点
  typedef list_node<T>*      node_ptr;  //可能会有扩展
};
...
{
  typedef typename node_traits<T>::base_ptr base_ptr;//重新定义为base_ptr
  typedef typename node_traits<T>::node_ptr node_ptr;//重新定义为node_ptr
  ...
  node_ptr as_node()//转换成node_ptr类型
  {
...
//   Czy 看懂了
// list 的迭代器设计，iterator的类型不用自己管，只需要输入T的类型即可
template <class T>
struct list_iterator : public mystl::iterator<mystl::bidirectional_iterator_tag, T>
{
  ...
  */
  ...
  self& operator++()//前++
  {
    MYSTL_DEBUG(node_ != nullptr); //assert
    node_ = node_->next;
    return *this;
  }
  self operator++(int)//后++
